    ...
    async function clickSubmit(e){
        ...
        if(pin.pin===pin.confirmPin){
            if(pin.pin==='' || pin.confirmPin===''){
                ...
                setPin({...pin, pin:'', confirmPin:'', isUpdateKey:true})
              
            }
        }
        else{
            setPin({...pin, isWarningKey:true, isBlank:false});
        }
            ...
            <div className={styles.inputContainer}>
                <form onSubmit={clickSubmit}>
                    ...
                    <div className='row'>
                        ...
                        <div>
                        <input onChange={(e)=>{setPin({...pin, pin:e.target.value, isWarningKey:false ,isBlank:false})}} value={pin.pin} name='pin' type="tel"  maxLength='4'/>
                        </div>
                    </div>
                    <div className='row mt-4'>
                        <label>Confirm New Pin</label>
                        <div>
                        <input onChange={(e)=>{setPin({...pin, confirmPin:e.target.value, isWarningKey:false, isBlank:false})}} value={pin.confirmPin} name='confirmPin' type="tel" maxLength='4'/>
                        </div>
                    </div>
                    <div className='row'>
